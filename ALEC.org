...
- Install the 32bit Emacs distribution straight from GNU
  - There are others. The 64-bit sounds pretty but I'm ignorant of the
    benefits and prefer a mainstream build
...
- Install [[http://aspell.net/win32/][Aspell Win32]]
  - Add to PATH
  - Manually install the English dictionary
  - Move =dict= and =data= contents/folders
- Keyboard setup
...
- Installed libpng from ezwinports for org display images inline
...
to satisfy my requirements, but then quit doing so. [[http://www.autohotkey.com/][AutoHotKey]] (AHK) now performs
the task acceptably.

I need to do the simplest possible thing here. I want to do it /the right way/ and
just deal with low-level key-codes. That is a poor use of time so I am not.
...
- I make WindowsKey act as to Super
- Issue is that Super keybindings activate default Windows actions
- Solution is to make WindowsKey instead send Meta
