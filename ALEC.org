...
Why not use =package-pinned-archives= here, since the contents are the same?

I don't recall. Originally I knew that I wanted to turn some packages on and off
and so I kept this list. Perhaps no other needs developed from there, so I did
...
#  LocalWords:  Savehist emacs fu API
