...
*.Renviron*
#+begin_src sh :tangle ~/.Renviron :noweb tangle
«renviron-def»
#+end_src
...
dplyr: dplyr: a grammar of data manipulation in R
...
#+begin_src ditaa :file auap.png :tangle no
...
For that matter, set up the way that I deal with nearly every =org= file ever:
...
#+begin_src sh :tangle bin/cpkf :tangle-mode (identity #o755) :noweb tangle
