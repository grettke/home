...
How to run:
- =.Renviron= is the important thing for telling what to go where
- First get everything installed
- Start R, but without the init file, run =.Rinstall= to get everything installed
- =r --no-init-file=
- Various things happen now
  - Java 6 gets installed
  - CRAN repo site chooser pops up
    - This is expected, .Rprofile specifies this
  - R exits
  - Just keep running the script until there are no issues and everything is
    installed
  - Ran it 5 times before there were no complaints
  - Look forward to automating this
- Very everything is installed correctly
- Restart it with the init file and make sure everyting looks right
